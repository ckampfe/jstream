...
impl<W: Write> PathValueWriter for Writer<'_, W> {
...
mod tests {
    ...
    #[test]
    fn one_json_jindex() {
        let s = std::fs::read("fixtures/one.json").unwrap();
    ...
    #[test]
    fn even_more_weird() {
        let s = std::fs::read("fixtures/city_lots_small.json").unwrap();
